...
public class DB_History_Nakladnaya_Update extends DBProcessor{
    public void update_h(String[][] mas){
        try (
                Connection connection = DriverManager.getConnection(URL, USERNAME, PASSWORD);
                Statement statement = connection.createStatement()) {
            for (int i = 0; i < mas.length; i++) {
                String sql_code = "update nakladnaya\n" +
                        "Set n_comment=? where nakladnaya_id = ?";
                PreparedStatement preparedStmt = connection.prepareStatement(sql_code);

                preparedStmt.setString(1, mas[i][1]);

                preparedStmt.setInt(2, Integer.parseInt(mas[i][0]));
